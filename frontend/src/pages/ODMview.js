...
function ODMview() {
  ...
  // 🔹 Apply filtering using Delivery ID & Trip Date
  // const filteredDeliveries = deliveries.filter((delivery) => {
  //   return (
  //     delivery.DeliveryID.toLowerCase().includes(searchQuery.toLowerCase()) &&
  //     (tripDateFilter === "" || delivery.TripDate === tripDateFilter)  // 🔹 Filtering by Trip Date
  //   );
  // });

  const filteredDeliveries = deliveries.filter((delivery) => delivery.TripDate.startsWith(searchDate));
