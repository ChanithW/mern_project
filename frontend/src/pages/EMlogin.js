...
function EMlogin() {
  ...
  const handleLogin = (e) => {
    ...
    if (username === "Empmanager" && password === "emp@2003") {
        navigate("/EmployeeDashboard"); // Redirect to the admin dashboard
    } else {
        alert("Invalid username or password");
        navigate("/EMlogin"); // Redirect to the admin login page
    }
  };
  ...
  return (
    <div className="flex justify-center items-center min-h-screen bg-cover bg-center" style={{ backgroundImage: `url(${bgImage})` }}>
       <Card className="w-96 p-6  bg-white shadow-lg rounded-lg">
        <h2 className="text-2xl font-bold text-center mb-4 text-black">Employee Manager Login</h2>
